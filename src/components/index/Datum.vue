	...
	<div class="datum">
		...
		<div class="test_select" v-show="!isChange">
  	    	<div class="test_select_cen2">
	  	    	<label>车牌号码</label>
			    <span class="inputs">
			    	<input type="text" placeholder="请输入车牌号" v-model="driverLicense" maxlength="7"  />
			    </span>
		    </div>
		</div>
		...
		<div class="test_select">
  	    	<div class="test_select_cen2">
			    ...
			    <!--<span class="inputs">
			    	<input type="text" placeholder="请输入车主姓名" v-model="name" />
			    </span>-->
		    </div>
		</div>
		<div class="test_select">
  	    	<div class="test_select_cen2">
			    ...
			    <!--<span class="inputs">
			    	<input type="text" placeholder="请输入车主身份证号" v-model="cardNo" />
			    </span>-->
		    </div>
		</div>
		<div class="xieyi">
	...
	export default{
		...
		data () {
		    return {
				...
				areaCode:'',  // 投保区域编码（选填）
				area:'',// 投保区域 （选填）
				driverLicense:'', // 车牌号  选填
				vehicleSign:'', // 辆是否已上牌（0、已上牌，1、未上牌，默认0）
				name:'',   // 投保人 
				cardNo:'',  // 投保人
				token:'',//获取token
	    ...
	    methods:{
		    ...
		    datumbuts(){
		    	...
		    	var group = {
					...
					"area":this.area, //区
					"driverLicense":this.isChange==false ? this.driverLicense : '', //车牌号
		    	...
		    	this.$http.post(this.$root.url+"/apply/applyPolicy",group , {
			        ...
			        if(data.body.ask==0){
			        	this.$router.push({path:'/yesbrand',query: {"Changeval":this.Changeval}})//其中login是你定义的一个路由模块
			        }
			    }, function(error) {// 这里是处理错误的回调
			        console.log(error)
			    });
