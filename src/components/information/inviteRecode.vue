	    ...
	    this.$http.post(this.$root.url+"userCenter/getInviterList",{
           ...
           if(res.ok&&res.body.ask==0&&res.body.data){
	           	for(var i=0;i<res.body.data.length;i++){
					...
					this.list.push({
						icon:res.body.data[i].avatarUrl,
...
<style scoped>
    ...
    .qcode-recode .r-item{padding: 10px 10px 15px;position: relative;background-color: #fff;}
    ...
    .qcode-recode .r-item .r-left{font-size: 0;float: left;}
    .qcode-recode .r-item .r-left .r-icon{width: 42px;height: 42px;border-radius: 50%;}
