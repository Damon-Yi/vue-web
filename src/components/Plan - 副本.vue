   ...
   <div>
      ...
      <div class="center_list">
         ...
         <div class="group_switch">
            <div  class="vux-x-switch weui-cell weui-cell_switch">
	         ...
	         <datetime v-model="TCIDate" v-show="TCI" placeholder="请选择起保日期" :start-date="TCIStartDate" required :title="'交强险保障生效期'" confirm-text="完成" cancel-text="取消"></datetime>
            <div  class="vux-x-switch weui-cell weui-cell_switch">
            	<div class="weui-cell__bd">
            ...
            <datetime v-model="VCIDate" v-show="VCI" placeholder="请选择起保日期" :start-date="VCIStartDate" required :title="'商业险保障生效期'" confirm-text="完成" cancel-text="取消"></datetime>
         </div>
         <!--主险、附加险-->
         <div class="alLists" >
            <!--主险-->
            <div class="checklist" v-show="VCI" >
	          	...
	          	<div class="vux_list_item" v-for="insur in mainInsurance">
	          		<div class="list_item_bd"><label>{{insur.name}}</label></div>
	          		<div class="list_item_inp" v-if="insur.widgetType=='selectInput'&&insur.value!==comTxt.unInsured">
                     <input type="number" placeholder="请输入投保金额" v-model="insur.inputVal"/>
                  </div>
	          		<div class="list_item_sele"> 
	          			<selector :options="insur.lib" v-model="insur.value" @on-change="dropDownChange">
	          			</selector>
	          		</div>
	          	</div>
            </div>
            ...
            <div class="checklist" v-show="VCI" >
	          	...
	          	<div class="vux_list_item" v-for="insur in additionalInsurance">
	          		<div class="list_item_bd"><label>{{insur.name}}</label></div>
	          		<div class="list_item_inp" v-if="insur.widgetType=='selectInput'&&insur.value!==comTxt.unInsured"   >
                     <input type="number" placeholder="请输入投保金额" v-model="insur.inputVal"/>
                  </div>
	          	...
	          	<div class="vux_list_item_txt" @click="IOPConfirm">
	          		<div class="list_item_bd"><label>不计免赔</label></div>
                  <div class="list_item_text" v-if="IOPItem.length>0">投保</div>
	          		<div class="list_item_text" v-else>不投保</div>
	          	</div>
            </div>
         </div>
         <div class="offerBut">
      ...
      <x-dialog v-model="showNoScroll" class="dialog-demo" :scroll="true">
         ...
         <div v-else-if="alertOptoion==2" class="swiper-box">
            <swiper height="210px">
               <swiper-item class="black" v-for="item in VCITipsList">
                  <div class="s-txt-box">
      ...
      <!--弹窗confirm-->
      <div class="confm">
	      <confirm v-model="IOPShowFlag" theme="ios" :title="'不计免赔'" cancel-text="返回">
	        	<checklist :options="IOPList" label-position="right" v-model="IOPItem"></checklist>
	      </confirm>
	    </div>
      <!--弹窗confirm-->
...
<script>
   ...
   import { Tab, TabItem, XButton, Swiper, SwiperItem , XSwitch, Group, Datetime, Checklist, Alert, Confirm, Selector, XDialog, Toast} from 'vux'
   ...
   export default {
      ...
      data () {
         return {
            ...
            alertShowFlag:false, //交强险、商业险 提示
            alertOptoion:'',
      ...
      methods: {
         ...
         getDataByCode (code){
            ...
            vm.$http.get(this.$root.url+"insurace/queryInsurance?code="+code,{
               ...
               if(res.ok&&res.body.ask==0&&res.body.data){
                  ...
                  //console.log('data.jsStartTime',data.jsStartTime);
                  //console.log('data.jsStartTime',vm.formateDate(new Date(data.jsStartTime)));
                  if(data.jsStartTime){//交强险生效时间
                     vm.TCIDate = vm.formateDate(new Date(data.jsStartTime));
                  }
                  ...
                  for(var i=0;i<data.plan.mainItems.length;i++){//主险
                     ...
                     }else if(data.plan.mainItems[i].widgetType=='selectInput'){
                        obj.value = data.plan.mainItems[i].flag?this.comTxt.insured:this.comTxt.unInsured;
                        obj.inputVal = data.plan.mainItems[i].value?data.plan.mainItems[i].value:data.plan.mainItems[i].defaultValue;
                  ...
                  for(var i=0;i<data.plan.extItems.length;i++){//附加险
                     ...
                     }else if(data.plan.extItems[i].widgetType=='selectInput'){
                        obj.value = data.plan.extItems[i].flag?this.comTxt.insured:this.comTxt.unInsured;
                        obj.inputVal = data.plan.extItems[i].value?data.plan.extItems[i].value:data.plan.extItems[i].defaultValue;
                     }else if(data.plan.extItems[i].widgetType=='cbox'){
                        obj.value = data.plan.extItems[i].flag?this.comTxt.insured:this.comTxt.unInsured;
                     }
                     vm.additionalInsurance.push(obj);
                  }
                  vm.IOPInsurance = data.plan.noCalcItems;//不计免赔
                  vm.IOPList = vm.creatIPOList(vm.IOPInsurance);
                  //if(data.planCode){
                     vm.IOPItem = vm.creatIPOItems(vm.IOPInsurance);
                  //}else{
                     //vm.IOPItem = vm.creatIPOList(vm.IOPInsurance);
                  //}
                  ...
                  console.log('IOPInsurance',JSON.stringify(vm.IOPInsurance));*/
               }else{
                  this.showTips = true;
         ...
         //不计免赔
         IOPConfirm () {
            this.IOPShowFlag = !this.IOPShowFlag
         },
         /*IOPConfirmFun () {
            console.log('IOPConfirmFun',this.IOPItem)
         },
         IOPChange (value) {
            console.log('IOPChange', value)
         },*/
         creatIPOList(data){//不计免赔 展示数据
            var obj = [];
         ...
         getIPOListData(){//整理不计免赔数据
            var obj = [];
            
            for(var k=0;k<this.IOPInsurance.length;k++){
               var tmp = {
                  code:this.IOPInsurance[k].code,
                  value:this.comTxt.unInsured,
                  flag:'false',
                  name:this.IOPInsurance[k].name
               };
               obj.push(tmp);
            }
            for(var j=0;j<obj.length;j++){
               for(var i=0;i<this.IOPItem.length;i++){
                  if(this.IOPItem[i] == obj[j].name){
                     obj[j].value = this.comTxt.insured;
                     obj[j].flag = 'true';
                  }
               }
            }
         ...
         generateSaveDate(){//整理保存数据
            ...
            parmas.hasJq = this.TCI;
            parmas.hasSy = this.VCI;
            parmas.jsStartTime = this.TCIDate;
            parmas.syStartTime = this.VCIDate;
            ...
            for(var i=0;i<this.mainInsurance.length;i++){
               ...
               }else if(this.mainInsurance[i].widgetType=='selectInput'){
                  tmp.value = this.mainInsurance[i].value!=this.comTxt.unInsured?this.mainInsurance[i].inputVal:this.comTxt.unInsured;
                  tmp.flag = tmp.value!=this.comTxt.unInsured?true:false;
            ...
            for(var i=0;i<this.additionalInsurance.length;i++){
               ...
               if(this.additionalInsurance[i].widgetType=='selectValue'){
                  ...
                  tmp.flag = tmp.value!=this.comTxt.unInsured?true:false;
               }else if(this.additionalInsurance[i].widgetType=='selectInput'){
                  tmp.value = tmp.value!=this.comTxt.unInsured?this.additionalInsurance[i].inputVal:this.comTxt.unInsured;
                  tmp.flag = tmp.value!=this.comTxt.unInsured?true:false;
            ...
            plan.noCalc = this.getIPOListData();
...
<style lang="less" scoped>
   ...
   .box {padding: 15px;}
   .active-6-1 {color: rgb(252, 55, 140) !important;border-color: rgb(252, 55, 140) !important;}
   .active-6-2 {color: #04be02 !important;border-color: #04be02 !important;}
   .active-6-3 {color: rgb(55, 174, 252) !important;border-color: rgb(55, 174, 252) !important;}
   .swiper-wrap{height: 600px;text-align: left;overflow-y:auto;}
   input, textarea {-webkit-appearance: none;outline: none;}
   .tab-swiper { background-color: #fff;}
   .vux-swiper{height: 200px;}
   .center {padding-top: 10px;padding-left: 15px;color: green;}
   .vux-slider .vux-swiper{overflow-y:auto;}
   ...
   .list_item_bd{-webkit-box-flex: 1;-ms-flex: 1;flex: 1;color:#7990a2}
   ...
   .list_item_sele{text-align: right;color:#999;width: 80px;}
...
<style>
   ...
   .list_item_sele .weui-select{height: auto;line-height:1;color: #434f5d;padding-right: 13px;}
